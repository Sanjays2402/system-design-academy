...
def landing_html(Q, HEAD):
    ...
    for index, q in enumerate(Q):
        ...
        cards.append(f'''<a class="design-card" data-category="{escape(category)}" href="designs/{slug}.html">
          ...
          <div class="card-meta"><span class="category">{escape(category)}</span><span class="card-depth">3.5K+ words · rationale · 2 diagrams</span></div>
        </a>''')
    path = ''.join(
    ...
    return f'''<!doctype html><html lang="en"><head>{HEAD}<title>System Design Academy · {len(Q)} Architecture Deep Dives</title><meta name="description" content="{len(Q)} system and low-level design deep dives with animated architecture, explicit trade-offs, and follow-up answers."><link href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500;600&display=swap" rel="stylesheet"><link rel="stylesheet" href="assets/style.css"><link rel="stylesheet" href="assets/landing.css"></head>
...
<main class="landing-wrap"><section class="hero-stage" aria-labelledby="landing-title"><div class="hero-copy"><div class="hero-kicker"><i class="live-dot"></i>{len(Q)} production systems · continuously deployable</div><h1 id="landing-title">Design the system.<span>Defend every choice.</span></h1><p class="hero-lede">An animated, long-form curriculum for learning why an architecture works, what alternatives were rejected, and how invariants, bottlenecks, failures, and cost shape each choice.</p><div class="hero-actions"><a class="hero-cta" href="designs/url-shortener.html">Start the curriculum <span class="arrow">→</span></a><a class="hero-cta secondary" href="#curriculum">Explore all systems</a></div><div class="hero-metrics"><div class="hero-metric"><b data-count="{len(Q)}">0</b><span>deep dives</span></div><div class="hero-metric"><b data-count="270">0</b><span>thousand words</span></div><div class="hero-metric"><b data-count="{len(Q)*2}">0</b><span>architecture diagrams</span></div></div></div>
...
<section class="curriculum-section" id="curriculum"><div class="section-heading"><div><span class="eyebrow">The curriculum</span><h2>{len(Q)} systems. No hand-waving.</h2></div><p>Filter by architecture family or search for a concept. Every design explains why the architecture was selected, rejected alternatives, trade-offs, evolution triggers, failure analysis, and answer guidance.</p></div>
...
<footer class="landing-footer"><p>Original architecture-learning artifact · {len(Q)} systems · explicit trade-offs · {len(Q)*2} custom diagrams.</p><p><a href="https://github.com/Sanjays2402/system-design-academy">Source on GitHub ↗</a></p></footer></main><script src="assets/site.js"></script><script src="assets/landing.js"></script></body></html>'''
