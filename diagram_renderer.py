...
def architecture_svg(title, spec):
    ...
    for i, (name, (x, y)) in enumerate(zip(nodes, positions)):
        ...
        boxes.append(f'''<g class="arch-node" data-node-index="{i}" data-kind="{kind}" data-component="{escape(name)}">
          <rect class="node-underlay" x="{x:.0f}" y="{y:.0f}" width="{box_w}" height="{box_h}" rx="12" style="fill:#07101f!important;stroke:none!important"/>
          <rect class="node-surface" x="{x:.0f}" y="{y:.0f}" width="{box_w}" height="{box_h}" rx="12" fill="{fill}" stroke="{color}"/>
    ...
    return f'''<svg viewBox="0 0 1280 {height}" role="img" data-component-count="{count}" aria-label="{escape(title)} detailed production architecture">
      ...
      <g class="call-flow-overlay" data-call-flow-overlay aria-hidden="true"></g>{layer_labels}{''.join(edges)}{''.join(boxes)}{legend}
    </svg>'''
