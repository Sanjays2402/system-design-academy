...
import json
from call_flow import build_flows
...
def render_chapter(item, idx, total, spec, architecture, sequence, head):
    ...
    decision_rows=''.join(_component_decision_row(name,i,spec,scale) for i,name in enumerate(spec['nodes']))
    decision_deep_dives=''.join(_decision_deep_dive(focus,i,title,spec,cat) for i,focus in enumerate(focuses))
    ...
    flow_json=escape(json.dumps(flows,separators=(',',':')),quote=True)
    flow_tabs=''.join(f'<button class="flow-tab {"active" if i==0 else ""}" data-flow-select="{flow["id"]}" aria-pressed="{"true" if i==0 else "false"}"><i style="--flow-color:{flow["color"]}"></i>{escape(flow["label"])}</button>' for i,flow in enumerate(flows))
    first_steps=''.join(f'<li class="{"active" if i==0 else ""}" data-static-flow-step="{i}"><b>{i+1:02d} · {escape(step["title"])}</b><span>{escape(step["text"])}</span></li>' for i,step in enumerate(flows[0]['steps']))
    return f'''<!doctype html><html lang="en" data-theme="dark"><head>{head}<title>{escape(title)} — System Design Academy</title><meta name="description" content="Production-grade system design tutorial for {escape(title)}."><link rel="stylesheet" href="../assets/chapter.css"><link rel="stylesheet" href="../assets/call-flow.css"><link rel="stylesheet" href="../assets/depth.css"></head><body class="chapter-page" data-default-theme="dark"><div class="chapter-container">
...
<section class="diagram-panel call-flow-panel" id="architecture" data-call-flow data-flows="{flow_json}"><div class="panel-title-row"><div><div class="panel-title">1. High-level architecture and animated call flow</div><p>Choose a path, then play or step through exactly how the call moves between components.</p></div><div class="flow-keyboard">← / → step · Space play</div></div>{architecture}<div class="flow-console"><div class="flow-toolbar" role="group" aria-label="Call flow controls"><div class="flow-tabs">{flow_tabs}</div><div class="flow-actions"><button data-flow-prev aria-label="Previous call-flow step">←</button><button data-flow-play aria-label="Play call flow">Play</button><button data-flow-next aria-label="Next call-flow step">→</button><button data-flow-restart aria-label="Restart call flow">Restart</button></div></div><div class="flow-status" role="status" aria-live="polite"><span data-flow-counter>Step 1 / {len(flows[0]['steps'])}</span><strong data-flow-title>{escape(flows[0]['steps'][0]['title'])}</strong><p data-flow-description>{escape(flows[0]['steps'][0]['text'])}</p></div><ol class="flow-static-list" data-flow-list>{first_steps}</ol></div></section>
...
<main class="tutorial"><h2>Complete System Design Tutorial</h2><p class="tutorial-lead">The design separates the latency-sensitive online path, the correctness-critical durable transition, and asynchronous projection or repair work. Each chapter below explains one decision shown in the diagrams.</p>
...
<section class="chapter" id="decisions"><h2>4. Why this architecture and what it costs</h2><p>The boxes are not a shopping list. Each boundary exists because it owns a different invariant, scaling dimension, data lifecycle, security policy, or failure domain. The table makes those choices explicit and shows when the design should remain simpler.</p><div class="table-wrap"><table><thead><tr><th>Component</th><th>Owned responsibility</th><th>Why separate it</th><th>Rejected alternative</th><th>Cost and evolution trigger</th></tr></thead><tbody>{decision_rows}</tbody></table></div><div class="design-rationale">{decision_deep_dives}</div><div class="callout good"><strong>Why this design:</strong> the synchronous path contains only the decisions required to protect {_invariant(cat,title).lower()} Durable projections, notifications, indexing, analytics, and repair are isolated so their load or failure cannot corrupt the acknowledged operation.</div>{core_details}<a class="back" href="#top">↑ top</a></section>
...
</main><footer>{escape(title)} system design · production architecture, critical sequences, and complete tutorial</footer><nav class="page-links"><a href="{f'{idx-1}.html' if False else '../index.html'}">← Curriculum</a><span>{idx+1:02d} / {total}</span></nav></div><script src="../assets/site.js"></script><script src="../assets/call-flow.js"></script></body></html>'''
...
def _rejected_component_alternative(n):
    n=n.lower()
    if any(x in n for x in ['cache','redis','cdn','index','projection']):return 'Read authoritative storage for every request. Simpler initially, but latency and source-of-truth load scale together and a hot key becomes a database incident.'
    if any(x in n for x in ['queue','stream','kafka','outbox','journal','log']):return 'Call every downstream consumer synchronously. That couples user latency and availability to analytics, notification, indexing, and audit dependencies.'
    if any(x in n for x in ['db','store','ledger','inventory','manifest','wal']):return 'Let each service persist its own interpretation. That removes one clear commit boundary and makes retries, support, and reconciliation ambiguous.'
    if any(x in n for x in ['gateway','api','edge','front door','kiosk']):return 'Put protocol, authentication, quotas, and business transitions in one process. Acceptable at low scale, but it couples external traffic risk to domain releases.'
    if any(x in n for x in ['worker','pipeline','fanout','repair','processor','transcode']):return 'Perform the work before acknowledging the caller. This gives simpler consistency but unbounded tail latency and fragile retries.'
    if any(x in n for x in ['audit','reconcil','metric','monitor','history']):return 'Infer correctness from request logs. Logs do not prove durable business state, detect silent drift, or safely repair ambiguous outcomes.'
    if any(x in n for x in ['auth','risk','policy','fraud','scanner','safety']):return 'Embed policy in every caller. Rules then drift by deployment version and emergency changes require fleet-wide redeployment.'
    return 'Keep this behavior inside an adjacent service. Prefer that simpler option until independent scale, ownership, policy, or failure isolation is measurable.'
def _component_cost_and_trigger(n,scale):
    n=n.lower()
    if any(x in n for x in ['cache','redis','cdn','index']):return f'Costs invalidation, staleness, stampede protection, and memory. Add or split it only after the workload ({scale}) proves repeated reads dominate.'
    if any(x in n for x in ['queue','stream','kafka','outbox','journal','log']):return 'Costs eventual consistency, schema evolution, replay tooling, idempotent consumers, and lag operations. Earn it when bursts or downstream failure must not block acknowledgment.'
    if any(x in n for x in ['db','store','ledger','inventory','wal']):return 'Costs schema migration, replication, backup, and shard ownership. Split storage only when access patterns, retention, or compliance boundaries materially differ.'
    if any(x in n for x in ['gateway','api','edge','front door']):return 'Costs another hop, policy distribution, and a high-blast-radius fleet. Introduce it when many clients or services need one protocol and protection boundary.'
    if any(x in n for x in ['worker','pipeline','fanout','repair','processor']):return 'Costs leases, checkpoints, retries, DLQs, and lag operations. Separate once execution time or burstiness no longer fits the request deadline.'
    return 'Costs an interface, deployment, telemetry, ownership, and failure mode. Keep it in-process first; extract when independent scale or reliability pays for the boundary.'
def _component_decision_row(name,i,spec,scale):
    previous=spec['nodes'][i-1] if i else 'the external caller'
    following=spec['nodes'][i+1] if i+1<len(spec['nodes']) else 'operators and repair tooling'
    lifecycle=spec['steps'][i%len(spec['steps'])]
    failure=spec['failures'][i%len(spec['failures'])]
    responsibility=f'{_component_role(name)} In this design it participates in: {lifecycle}'
    why=f'{_why_component(name)} It prevents {previous} from also owning the contract and failure behavior of {following}.'
    rejected=f'{_rejected_component_alternative(name)} Here that simpler choice would couple “{failure}” to {previous}.'
    cost=f'{_component_cost_and_trigger(name,scale)} Measure the latency, saturation, lag, or mismatch at the {name} boundary before extracting or scaling it.'
    return f'<tr><td><strong>{escape(name)}</strong></td><td>{escape(responsibility)}</td><td>{escape(why)}</td><td>{escape(rejected)}</td><td>{escape(cost)}</td></tr>'
def _decision_deep_dive(focus,i,title,spec,cat):
    alternatives=[
      ('Chosen boundary vs simpler monolith',f'A single deployable with one transactional store is the right starting point for a small {title.lower()} workload. The proposed boundary becomes justified when {focus.lower()} Separating it contains the named failure “{spec["failures"][i%len(spec["failures"])]}” without placing unrelated work on the critical path.'),
      ('Correctness vs availability',f'The design coordinates only where {_invariant(cat,title).lower()} Everywhere else it accepts bounded staleness or asynchronous propagation. The alternative—strong coordination for every projection—simplifies the mental model but increases tail latency and turns partitions into broad write unavailability.'),
      ('Operational cost and evolution',f'This choice adds dashboards, ownership, schema compatibility, capacity planning, and recovery runbooks. Start with the minimum topology that supports “{spec["steps"][i%len(spec["steps"])]}” and extract another service only when measured throughput, blast radius, data lifecycle, or team ownership changes independently.')]
    heading,body=alternatives[i%len(alternatives)]
    return f'<article class="rationale-card"><span>Decision {i+1:02d}</span><h3>{escape(heading)}</h3><p>{escape(focus)}</p><p>{escape(body)}</p></article>'
